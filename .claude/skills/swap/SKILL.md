...
- **Pułap kosztowy: domyślnie nie wychodź powyżej Sonnet 5.** Piotr świadomie ogranicza koszty na czas przechodzenia przez kurs — Claude Opus 5 i Claude Fable 5 są zauważalnie droższe i NIE mają się pojawiać w generowanym kodzie jako model domyślny, nawet jeśli oryginalny kod OpenAI wywoływał swój najdroższy/flagowy model (np. `gpt-5.4` bez sufiksu). Gdy notatnik pokazuje progresję tani→średni→flagowy, mapuj górny poziom na model z tej samej warstwy co Sonnet 5 (nie na warstwę Opus/Fable) — dwa poziomy (tani/szybki + Sonnet-warstwa) w zupełności oddają ideę "słabszy vs mocniejszy model" bez sięgania po najdroższe modele. Ta zasada dotyczy warstwy modelu (którą warstwę wybrać), nie konkretnego ID — samo ID nadal bierz z `claude-api`, żeby nie zapisywać go tutaj na sztywno. Odstępstwo: rób to tylko wtedy, gdy Piotr wyraźnie poprosi o konkretny model (np. "użyj tu Opusa") albo zadanie w danym labie realnie wymaga zdolności poza warstwą Sonnet — w takim przypadku zapytaj/zaznacz to wprost zamiast cicho przełączać na droższy model.
